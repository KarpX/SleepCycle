        ...
        return MaterialApp(
          ...
          theme: ThemeData.light().copyWith(
            colorScheme: ColorScheme.light(
              ...
              secondary: const Color.fromARGB(255, 218, 205, 253),
              onSurface: const Color.fromARGB(255, 103, 80, 164),
              surface: const Color.fromARGB(255, 210, 194, 255),
              onPrimaryContainer: const Color.fromARGB(255, 248, 246, 255),
            ),
            primaryColorLight: const Color.fromARGB(255, 239, 234, 255),
            primaryColorDark: const Color.fromARGB(255, 66, 38, 114),
            cardColor: const Color.fromARGB(255, 103, 80, 164),
          ),
          darkTheme: ThemeData.dark().copyWith(
            colorScheme: ColorScheme.dark(
              primary: Color.fromARGB(255, 99, 76, 142),
              secondary: Color.fromARGB(255, 40, 30, 56),
              ...
              surface: const Color.fromARGB(255, 29, 22, 40),
              onPrimaryContainer: const Color.fromARGB(255, 77, 59, 111),
            ),
            primaryColor: Color.fromARGB(255, 99, 76, 142),
            primaryColorLight: const Color.fromARGB(255, 189, 159, 244),
            primaryColorDark: Color.fromARGB(255, 63, 47, 92),
            cardColor: const Color.fromARGB(255, 99, 76, 142),
          ),
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  late TimeOfDay _selectedSleepingTime;
  ...
  void initState() {
    ...
    _selectedSleepingTime = TimeOfDay.fromDateTime(DateTime.now());
    _checkPermissons();
        ...
        return Theme(
          data: ThemeData.light().copyWith(
            timePickerTheme: TimePickerThemeData(
              ...
              dialBackgroundColor: theme.colorScheme.primary,
              hourMinuteColor: theme.colorScheme.primary,
              ...
              entryModeIconColor: theme.colorScheme.onSurface,
            ),
  ...
  Future<void> _selectSleepingTime(BuildContext context) async {
    final TimeOfDay? picked = await showTimePicker(
      context: context,
      initialTime: _selectedSleepingTime,
      builder: (BuildContext context, Widget? child) {
        final theme = Theme.of(context);
        return Theme(
          data: ThemeData.light().copyWith(
    ...
    if (picked != null && picked != _selectedSleepingTime) {
      setState(() {
  ...
  Future<void> _scheuleSleepReminder(TimeOfDay time) async {
    ...
    await NotificationsService.scheduleNotification(
      ...
      scheduledTime: DateTime.parse(notificationsList[id]!['time']),
      days: (notificationsList[id]!['days'] as List)
          .map((e) => e as int)
          .toList(),
    );
  ...
  void _saveNotification(TimeOfDay time, List<int> days) {
  ...
  void generateSleepingTimeList() {
    setState(() {
      timeList.clear();

      DateTime now = DateTime.now();
      debugPrint("selectedTime: $_selectedSleepingTime");
      DateTime curr = DateTime(
        now.year,
        now.month,
        now.day,
        _selectedSleepingTime.hour,
        _selectedSleepingTime.minute,
      );
      for (int i = 0; i < countItems; i++) {
        ...
        if (enableBeforeSleepTime) {
          DateTime beforeSleep = curr.add(Duration(minutes: beforeSleepTime));
          timeList.add(
  ...
  Widget _buildCurrentScreen() {
    switch (_currentMode) {
      case AppMode.awakening:
        generateTimeList();
        return _buildAwakingScreen();
      case AppMode.sleeping:
        generateSleepingTimeList();
        return _buildSleepingScreen();
    }
  }
    ...
    return Center(
      child: Column(
        ...
        children: <Widget>[
          ...
          BigCard(selectedTime: _selectedSleepingTime),
          SizedBox(height: 20.h),
          ElevatedButton(
            style: ButtonStyle(
                  ...
                  return InkWell(
                    onTap: () {
                      int id =
                          timeList[index].hour * 60 + timeList[index].minute;
                      if (!notificationsList.containsKey(id)) {
                        _saveNotification(timeList[index], []);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: "Настройки",
        child: Icon(Icons.settings, color: theme.colorScheme.onPrimary),
      ),
                                    ...
                                    if (isSelectedMode) {
                                      ...
                                      if (result != null) {
                                        setState(() {
                                          ...
                                          _checkPermissionsAndSchedule(
                                            TimeOfDay.fromDateTime(
                                              DateTime.parse(
                                                notificationsList[id]!['time'],
                                              ),
                                            ),
                                          );
                                        });
  ...
  Future<void> _enableNotification(int id) async {
    ...
    notificationsList[id]!['time'] = time.toString();
    _saveNotificationsList(notificationsList);
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SwitchListTile(
                      ...
                      thumbColor: enableBeforeSleepTime
                          ? WidgetStatePropertyAll(
                              Theme.of(context).colorScheme.surface,
                            )
                          : WidgetStatePropertyAll(
                    ...
                    //   },
                    //   child: Text(notificationsList.toString()),
                    // ),
                    SizedBox(
